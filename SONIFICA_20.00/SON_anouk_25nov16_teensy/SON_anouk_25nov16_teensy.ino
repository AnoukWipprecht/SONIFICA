#include <Audio.h>	//Resource => http://www.pjrc.com/teensy/td_libs_Audio.html
#include <Wire.h>
#include <SPI.h>
#include <SD.h>
...
// Bounce objects to read six pushbuttons
// might be useful for testing - manaully trigger the samples
//
Bounce button0 = Bounce(2, 5);
Bounce button1 = Bounce(3, 5);  // 5 ms debounce time
Bounce button2 = Bounce(4, 5);
Bounce button3 = Bounce(5, 5);
Bounce button4 = Bounce(6, 5);
Bounce button5 = Bounce(8, 5);
...
void loop() {
	...
	if (button2.fallingEdge()) {
	  sound2.play(AudioSampleA01);
	}
	if (button3.fallingEdge()) {
	  sound3.play(AudioSampleA02);
	}
	if (button4.fallingEdge()) {
	  sound4.play(AudioSampleA03);
	}
	if (button5.fallingEdge()) {
	  sound5.play(AudioSampleKick);
	}
	/* DEBUG */

	if (Serial1.available() && 
		!sound0.isPlaying() && !sound1.isPlaying()) {
		char c = Serial1.read();
		/* DEBUG */
		...
		if (c == 'A') {
			...
			sound0.play(AudioSampleA01);
		} else if (c == 'C') {
			sound0.play(AudioSampleA03);
		} else if (c == 'E') {
			sound1.play(AudioSampleSnare);
		} else if (c == 'F') {
			sound1.play(AudioSampleA02);
		} else if (c == 'G') {
			sound1.play(AudioSampleA04);
		} else {
			// No replay
			;
		}
